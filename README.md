...
| `TestProject/`       | A Python directory that contains code for your Dagster repository                 |
| `TestProject_tests/` | A Python directory that contains tests for `TestProject`                      |
...
pyenv virtualenv $PYTHON_VERSION TestProject
...
```bash
export PYTHON_VERSION=X.Y.Z
conda create --name TestProject python=PYTHON_VERSION
...
Tests can be found in `TestProject_tests` and are run with the following command:

```bash
pytest TestProject_tests
```

As you create Dagster ops and graphs, add tests in `TestProject_tests/` to check that your
